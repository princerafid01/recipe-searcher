...
async function fetchAndDisplay(query) {
  ...
  singleIngre.forEach(ingre => {
    ingre.addEventListener('change', e => {
      if (e.currentTarget.checked && !searchIngre.includes(e.currentTarget.value)) {
        searchIngre.push(e.currentTarget.value);
      }

      if (!e.currentTarget.checked) {
        searchIngre = searchIngre.filter(ingree => ingree !== e.currentTarget.value);
      }
...
allIngredients.forEach(ingredient => {
  ingredientEl.insertAdjacentHTML('afterbegin', `
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "43763" + '/');
